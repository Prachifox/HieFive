...
<center><h4><span class='teal-text'></span>Followers</h4></center>
<?php
if(isset($_GET['u_id']))
{
  $userr_id=$_GET['u_id'];
    ...
    $follow="select * from follow where following='$user_logged'";
    $run_follow = mysqli_query($con,$follow);
      echo"<br><div class='row'>";
  $i=1;
    while($row_follow=mysqli_fetch_array($run_follow))
    {
      $follower_id=$row_follow['follower'];
      $get_userzz="select * from users where user_id='$follower_id'";
...
if($userr_id==$user_ids)
{
echo"<form method='post'>
<button class='btn red lighten-4' name='delete' type='submit' style='float:right;'><i class='fa fa-trash red-text small' ></i></button></form>";
}
echo"<a href='user_profile.php?u_id=$user_id'><span class='black-text center'>
<h5><i class='fa fa-address-card teal-text'></i> $user_name</h5></span>
</div></div>";
if($i%2==0)
...
if(isset($_POST['delete']))
{
  ...
  $delete_follow="delete from follow where (follower='$user_id' AND following='$u_id')";
   $run_delete=mysqli_query($con,$delete_follow);
if($run_delete)
{
 
  echo"<script>alert('Follower successfully removed')</script>";
echo"<script>window.open('followers.php?u_id=$u_id','_self')</script>";
}
}
?>